...
struct layer* init_layer(int t, double p){
...
void free_layer(struct layer* s){
    int j;
    for (j=0; j<s->size; j++)
        free(s->v[j]);
    free(s->v);
                    ...
                    layer* random_layer)
{
    ...
    for (i=0; i<size; i++) {
        for (j=0; j<size; j++) {
            for (n=0; n<octaves; n++) {
                current_layer->v[i][j]+=mes_layers[n]->v[i][j]*mes_layers[n]->persistance;
            }
            // normalisation
            current_layer->v[i][j] =  current_layer->v[i][j] / sum_persistances;
        }
    }
